import { AlertCircle, Bot, Calendar, Download, FileText, History, RotateCcw, User } from 'lucide-react';
                  ...
                  >
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <span className={`font-bold text-lg ${isCurrentVersion ? 'text-primary' : ''}`}>v{v.version}</span>
                        {isCurrentVersion && <span className="text-xs bg-primary text-primary-foreground px-2 py-1 rounded">Current</span>}
                      </div>

                      <div className="space-y-1 text-sm">
                        {/* Creator */}
                        <div className="flex items-center gap-2 text-muted-foreground">
                          {creatorInfo.isAgent ? <Bot className="h-3.5 w-3.5" /> : <User className="h-3.5 w-3.5" />}
                          <span>
                            {creatorInfo.displayName}
                            {creatorInfo.isAgent && <span className="ml-1 text-xs opacity-70">(agent)</span>}
                          </span>
                        </div>
                        ...
                        {/* Size */}
                        <div className="text-muted-foreground">Size: {formatSize(v.size)}</div>

                        {/* Change reason */}
                        {v.change_reason && <div className="text-muted-foreground italic mt-1">"{v.change_reason}"</div>}
                      </div>
                    </div>

                    {/* Actions */}
                    <div className="flex gap-2 ml-4">
                      <Button variant="ghost" size="sm" onClick={() => handleDownloadVersion(v.version)} title="Download this version">
                      ...
                      {!isCurrentVersion && (
                        ...
                        >
                          {rollingBack === v.version ? <span className="text-xs">Rolling back...</span> : <RotateCcw className="h-4 w-4" />}
                        </Button>
                      )}
                    </div>
                  </div>
          ...
          {!loading && versions.length > 0 && (
            <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg p-3 text-sm mt-4">
              <p className="text-blue-900 dark:text-blue-100">
                <strong>💡 Version History:</strong> All file edits are tracked with the user or agent who made the change. You can download any version or
                rollback to restore previous content.
              </p>
            </div>
          )}
        </div>
